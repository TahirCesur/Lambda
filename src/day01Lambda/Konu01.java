...
public class Konu01 {
    ...
    public static void main(String[] args) {
        ...
        // lambda--functional programming --> nasil yaparim degil ne yaparim sorulur...
        // java da ise ne yaprimdan cok nasil yaparimi dusundurur...


        // Lambda da kod kisaligi kod okunabilirligi ve hatasiz kullanim acisaindan kolaylik saglarken
        // Java da ise kod genelde uzundur ve okunmasi ve kullanimi zordur...

        // lambda collectionslar da ve array lerde kullanilir mapler de kullanilmaz..


        // javada Lambda fonksiyonları (metodları) sadece Fonksiyonel Interface ler
        // (lambda da bulunan metodlar=> filter, reduce  ...gibi)
        ...
        // Stream (akış) bir API'dir ve nesne koleksiyonlarını işlemek için kullanılır.
        ...
        // Orneğin:elemanların karesini almak... map(t-> t*t)(4 elemanını=>16 yapar)
        // map ile Math class ındaki metodları kullanabiliriz...map(Math::sqrt)==> burada elemanların karesini almışız
        // map(String::toUpperCase)==> String class ından metod çağırabiliriz...
        // list imizin data type ının class ındaki metodlari kullanabiliriz.

        /*
        ==> t-> arrow fuction

        ==> t -> t % 2 == 0 ===> lambda expression

        ==> filter() inside lambda expression ==>>> filtrelemeye yarayan methoddur.

        ==> map() inside lambda expression ==>>> değişiklikler yapmaya yarayan method.

        ==> distinct() ==>>>  tekrarlı elemanları siler

        ==> foreach() ==>>>  yazdırma bitiriş elemanı(terminal bitiş elemanı)

        ==> reduce() çoklu elemanı 1 elemana düşürmek için kullanılır. (terminal bitiş elemanı) kullanılışı ===>>> reduce(0 , (t,u)-> x+y)
        *** 0 başlangıç değeridir. toplam için 0 çarpım için etkisiz eleman olan 1 i seçeriz. (t,u) kullanacağımız, atama yapacağımız elemanlar.
        ...
        *** Math clasından methodları kullanırkende Math::addExatc gibi multplyExatc gibi methodları kullanabiliriz.(reduce içinde kullanılabilir)

        ==> collect(Collectors.toList()) ==>>>> burada liste çevirdik. istersek bunu bir liste assign edebiliriz ya da yazdırdığımız methodun return type ını list<Integer> yaparak return yapabiliriz.

        ==> limit () içine koyduğumuz değer kadar eleman getirir. fakat baştan başlar. baştan ilk 4 eleman dersek 1,2,3,4 elemanlarını getirir.

        ==> skip() içine koyduğumuz değer kadar elemanı atlar  ve diğerlerini getirir. baştan ilk 4 elemanı atladık 5,6,7,8,9 elemanlarını getirir.
